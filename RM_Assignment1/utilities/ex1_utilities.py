...
def get_discount_factor_by_zero_rates_linear_interp(
    ...
    if len(dates) != len(discount_factors):
        raise ValueError(
            "Dates and discount factors must have the same length"
        )

    year_fractions = [
        year_frac_act_x(reference_date, T, 365) for T in dates[1:]
    ]
...
def date_series(
    ...
    while dates[-1] < t1:
        dates.append(
            business_date_offset(t0, month_offset=len(dates) * 12 // freq)
        )
    if dates[-1] > t1:
        dates.pop()
    if dates[-1] != t1:
...
def swaption_price_calculator(
    ...
    d1 = 1/(sigma_black*np.sqrt(ttm))*np.log(S0/strike) + 0.5*sigma_black*np.sqrt(ttm) # noqa
    ...
    deltas = [
        ...
        for prev, next in zip(
            fixed_leg_payment_dates[:-1], fixed_leg_payment_dates[1:]
        )
    ]
    ...
    if compute_delta:
        if swaption_type == SwapType.RECEIVER:
            ...
            return B_interp[0]*BPV*(strike*norm.cdf(-d2)-S0*norm.cdf(-d1)), delta   # noqa
        else:
            pass
...
def irs_proxy_duration(
    ...
    yf = [
        yearfrac(ref_date, d, DayCount.EU_30_360)
    ...
    deltas = [
        yearfrac(prev, next, DayCount.EU_30_360)
        for prev, next in zip(
            fixed_leg_payment_dates[:-1],
            fixed_leg_payment_dates[1:]
        )
    ]
    deltas.insert(
        0, yearfrac(ref_date, fixed_leg_payment_dates[0], DayCount.EU_30_360)
    )

    numerator = swap_rate * sum(
        [deltas[i] * yf[i] * discounts_duration[i] for i in range(len(yf))]
        ) + yf[-1] * discounts_duration[-1]

    denominator = swap_rate*sum(
        deltas[i]*discounts_duration[i]
        for i in range(len(deltas))) + discounts_duration[-1]
...
def swap_par_rate(
    ...
    if fwd_start_date is not None:
        deltas = [
            yearfrac(prev, next, DayCount.EU_30_360)
            for prev, next in zip(
                fixed_leg_schedule[:-1], fixed_leg_schedule[1:]
            )
        ]
        deltas.insert(
            0, yearfrac(fwd_start_date, fixed_leg_schedule[0], DayCount.EU_30_360)  # noqa
        )

        B_interp = [get_discount_factor_by_zero_rates_linear_interp(
            today,
            date,
            discount_factors.index,
            discount_factors.values
        ...
        deltas.insert(
            0, yearfrac(today, fixed_leg_schedule[0], DayCount.EU_30_360)
        )
...
def swap_mtm(
    ...
    deltas = [yearfrac(prev, next, DayCount.EU_30_360)
              for prev, next in zip(fixed_leg_schedule[:-1], fixed_leg_schedule[1:])] # noqa
...
def shift_curve(b: Bootstrap, bp: float = 1e-4,
                depos=True, futures=True, swaps=True) -> Bootstrap:
    """
